...
<body class="bg-gray-100">
    ...
    <div class="font-sans p-6 mx-auto lg:max-w-5xl md:max-w-3xl sm:max-w-full bg-white rounded-lg shadow-md">
        <h2 class="text-4xl text-center font-bold text-gray-800 mb-8">Brands List</h2>
        <div>
            @if (@session('Success'))
            ...
            <table class="min-w-full bg-white divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-100 transition duration-200">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{{ $brand->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $brand->id }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('editbrand', $brand->id) }}" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2.5 me-2 mb-2 transition duration-200">
                                Edit
                            </a>
                            <form action="{{ route('deletebrand', $brand->id) }}" method="POST" style="display:inline;" onsubmit="return confirmDelete();">
                                @csrf
                                @method('DELETE')
